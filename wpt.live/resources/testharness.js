...
(function (global_scope) {
    ...
    function test(func, name, properties) {
        ...
        if (value !== undefined) {
            var msg =
                'Test named "' + test_name + '" passed a function to `test` that returned a value.';
    ...
    function step_timeout(func, timeout) {
        ...
        var local_set_timeout =
            typeof global_scope.setTimeout === "undefined" ? fake_set_timeout : setTimeout;
    ...
    const formatEscapePattern = new RegExp(
        `[${Object.keys(formatEscapeMap)
            .map(k => (k === "\\" ? "\\\\" : k))
            .join("")}]`,
        "g"
    );
    ...
    function format_value(val, seen) {
        ...
        switch (typeof val) {
            case "string":
                return (
                    '"' + val.replace(formatEscapePattern, match => formatEscapeMap[match]) + '"'
                );
            case "boolean":
            case "undefined":
                return String(val);
    ...
    function assert_readonly(object, property_name, description) {
        ...
        while (
            object &&
            (desc = Object.getOwnPropertyDescriptor(object, property_name)) === undefined
        ) {
        ...
        if (desc.hasOwnProperty("value")) {
            ...
            assert(
                ...
                "assert_readonly",
                description,
                "descriptor [[Writable]] expected false got ${actual}",
                { actual: desc.writable }
            );
    ...
    Test.prototype.step = function (func, this_obj) {
        ...
        try {
            ...
            var status =
                e instanceof OptionalFeatureUnsupportedError ? this.PRECONDITION_FAILED : this.FAIL;
    ...
    Output.prototype.setup = function (properties) {
        ...
        this.enabled =
            this.enabled &&
            (properties.hasOwnProperty("output") ? properties.output : settings.output);
    };
        ...
        if (!node) {
            ...
            if (output_document.body) {
                ...
                if (is_svg) {
                    ...
                    root.appendChild(
                        output_document.createElementNS("http://www.w3.org/1999/xhtml", "body")
                    ).appendChild(node);
                } else {
        ...
        if (this.output_node && !tests.hide_test_state) {
            if (
                ...
                this.output_node.textContent =
                    "Running, " + done_count + " complete, " + tests.num_pending + " remain";
            }
        }
    };
            ...
            on_event(element, "click", function (e) {
                ...
                if (!style_element && !input_element.checked) {
                    ...
                    style_element.textContent =
                        "table#results > tbody > tr.overall-" + result_class + "{display:none}";
    ...
    function make_message(function_name, description, error, substitutions) {
        ...
        var node_form = substitute(
            ...
            merge(
                { function_name: function_name, description: description ? description + " " : "" },
                substitutions
            )
        );
