...
function test_computed_value(property, specified, computed, titleExtra, options = {}) {
    ...
    test(() => {
        ...
        assert_true(
            property in getComputedStyle(target),
            property + " doesn't seem to be supported in the computed style"
        );
        assert_true(
            CSS.supports(property, specified),
            "'" + specified + "' is a supported value for " + property + "."
        );
...
function testComputedValueGreaterOrLowerThan(property, specified, expected, titleExtra) {
    test(() => {
        ...
        if (expected > 0) assert_greater_than_equal(readValue, expected, specified);
...
function test_pseudo_computed_value(pseudo, property, specified, computed, titleExtra) {
    ...
    test(() => {
        ...
        try {
            ...
            assert_true(
                property in getComputedStyle(target, pseudo),
                property + " doesn't seem to be supported in the computed style"
            );
            assert_true(
                CSS.supports(property, specified),
            ...
            if (readValue !== specified) {
                ...
                assert_equals(
                    getComputedStyle(target, pseudo)[property],
                    readValue,
                    "computed value should round-trip"
                );
            }
